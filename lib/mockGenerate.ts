...
**Next Steps:** Align on joint targets for next quarter, resolve open blockers, confirm 8x8 support commitments.`;
  }

  if (modifier === "executive") {
    return `## Executive Briefing — ${partner} QBR (${quarter})

${partner} delivered a productive quarter with measurable pipeline growth and program progression. Key wins demonstrate deepening alignment with 8x8's go-to-market and a growing ability to deliver better outcomes for shared customers. Risks are addressable with targeted support.

**Headline:** On track — 1–2 areas require focused 8x8 support to protect Q3 forecast.
...
| # | Action | Owner | Due |
|---|--------|-------|-----|
| 1 | Confirm next quarter pipeline and revenue targets | 8x8 Partner Manager | End of week |
| 2 | Resolve blockers: ${challenges || "TBD"} | 8x8 internal team | 2 weeks |
| 3 | Follow up on partner asks: ${asksFromPartner || "TBD"} | 8x8 Partner Manager | 1 week |
| 4 | Internal escalation: ${internalAsks || "align with channel leadership"} | 8x8 stakeholders | 2 weeks |
| 5 | Schedule next QBR and set agenda | Both parties | This call |`;
  }

  return `## ${partner} — 8x8 Quarterly Business Review (${quarter})

---

### Executive Summary
${partner} demonstrated solid progress this quarter in their 8x8 partnership. Pipeline is growing, program milestones are being hit, and the relationship is trending in the right direction. There are 1–2 targeted areas where 8x8 support will be critical to protecting the forecast and delivering strong customer outcomes heading into next quarter.
...
- Lock in next quarter revenue and seat targets — break out 8x8 Work, 8x8 Contact Center, and 8x8 Engage attach goals separately
- Escalate internal asks to the appropriate 8x8 teams with clear owners and deadlines
...
2. Review pipeline with discipline — quality over quantity, and identify where 8x8 can accelerate
3. Be direct about risks — what does the partner need from 8x8 to unblock?
4. Partner asks: ${asksFromPartner}
5. 8x8 asks: ${internalAsks}
...
function generateBizPlan(data: Record<string, string>, modifier: Modifier): string {
  ...
  if (modifier === "shorter") {
    return `## ${partner} — 8x8 Joint Business Plan (${horizon})

**Goal:** ${goal} | **Type:** ${type} | **Segment:** ${segment}

**8x8 Solutions:** ${solutions}
...
**30 Days**
- [ ] Complete partner onboarding and 8x8 Elevate Partner Program agreement review
- [ ] Identify first 3–5 target accounts in ${segment}
- [ ] Begin enablement on ${solutions} — certification plan agreed
- [ ] Register first pipeline opportunities in 8x8 Partner Portal
...
- [ ] Confirm plan for remainder of ${horizon}`;
  ...
  return `## 8x8 Joint Partner Business Plan — ${partner}

---

### Business Plan Summary
This plan outlines the joint go-to-market strategy between 8x8 and ${partner}, a ${type}, targeting ${segment} over ${horizon}. The goal is to build a productive, scalable 8x8 practice together — helping customers improve how their employees communicate, how their teams serve customers, and how AI can create better business outcomes — with clear milestones and mutual accountability.
...
- **Priority 8x8 Solutions:** ${solutions}
...
Recommended actions:
- 8x8 product and solution training — 8x8 Work, 8x8 Contact Center, and 8x8 Engage
- Contact Center SE certification through 8x8 Partner University
- Deal registration process training — 8x8 Partner Portal walkthrough
- Access to demo environments and 8x8 sales tools
...
- 8x8 Engage demo event for customer-facing teams — especially high-impact in financial services, healthcare, and retail
...
**30 Days:** Complete enablement kickoff, align on target accounts, register first pipeline in 8x8 Partner Portal, confirm MDF and SPIFF plan.
...
| # | Action | Owner | Due |
|---|--------|-------|-----|
| 1 | Follow up on open questions: ${openQ} | 8x8 Partner Manager | [Date TBD] |
| 2 | Confirm decisions in writing: ${decisions} | Both parties | This week |
| 3 | Send next meeting agenda | 8x8 Partner Manager | 3 days before ${nextMeeting} |
| 4 | Internal alignment on any commitments made | 8x8 internal | Before ${nextMeeting} |`;
  }
...
**Subject:** Follow-Up: ${partner} / 8x8 — ${date}
...
I'll take care of my action items on the 8x8 side and follow up with you no later than [date]. Please let me know if anything has changed or if I missed anything from our conversation.
...
4. Any new asks or support needed from 8x8
...
**Recommended Approach:** Lead with business outcomes and the risk of staying on the current path. Position 8x8 as a strategic platform decision that improves customer experience, simplifies operations, and enables AI — not a vendor swap. Engage at the economic buyer level before the RFP closes.`;
  }
...
- [ ] Request executive introduction from 8x8 channel leadership if needed
- [ ] Schedule discovery call focused on: ${opportunity}
- [ ] Pull competitive intelligence if RFP is in play
- [ ] Loop in 8x8 SE if technical evaluation is underway`;
  }
...
- Send a relevant 8x8 customer story from ${industry}
- Schedule discovery call focused on ${opportunity}
- Loop in 8x8 SE if technical evaluation or proof of concept is expected`;
}
...
**Recommended 8x8 Action:** Prioritize exec and SE support on top 2 deals. Address stalled situations with direct outreach. Approve support requests to protect the quarter.`;
  }
...
|---|--------|-------|----------|
| 1 | Review stalled deals and identify re-engagement path: ${stalled} | 8x8 Partner Manager | High |
| 2 | Confirm close plan and 8x8 support for top opportunities | Partner AE + 8x8 PM | High |
| 3 | Escalate support needs to 8x8 internal: ${support} | 8x8 Partner Manager | High |
...
These deals represent the highest probability and value in the current pipeline. 8x8 resources — SE, exec, deal desk — should be prioritized here first to protect the quarter.
...
These situations require active intervention. Common causes: champion change, budget pause, or competitive pressure. Recommended: direct 8x8 outreach and a refreshed business case focused on the customer outcomes the deal delivers.

<!-- MANAGER_NOTES -->

### Recommended Actions
1. Weekly deal reviews on top 3 opportunities — confirm next step and who owns it
2. 8x8 executive outreach on stalled deals to reset conversations at the right level
3. Escalate support requests to 8x8 internal teams — see support needs below
4. Address forecast exposure: ${concerns}
5. Update deal registration for all active opportunities in 8x8 Partner Portal
...
4. What's the one thing 8x8 could do this week that would most accelerate your pipeline?
...
**How to deploy:** Share in next partner call or enablement session. Reinforce in QBR talking points. Available on 8x8 Partner Portal.`;
  }

  if (modifier === "action_items") {
    return `## Distribution & Activation Checklist — ${topic}

- [ ] Review and approve draft with 8x8 Product Marketing
- [ ] Remove any internal-only pricing or roadmap references
- [ ] Upload to 8x8 Partner Portal under the appropriate solution category
...
- [ ] Collect feedback after 30 days of partner use and refine`;
  ...
  return `## ${topic} — ${format}
**Audience:** ${audience} | **8x8 Solution:** ${solution}
...
**Opening:**
"The customers we're seeing right now are dealing with a common problem — their employees and their contact center agents are on different platforms. That means disconnected experiences for customers, fragmented data, and a support model that's nobody's full responsibility. 8x8 solves that. Here's how to position it."

**Value Statement:**
8x8 gives your customers a connected experience from the front line to the back office — **8x8 Work** for every employee, **8x8 Contact Center** for every agent, and **8x8 Engage** for AI-powered customer interactions. The result: faster service, simpler operations, and a communications foundation that makes AI outcomes possible. One platform, one SLA, backed by 8x8's 99.999% uptime guarantee.

**On 8x8 AI Studio:**
"One more thing your customers can't get from Avaya or most of our competitors — 8x8 AI Studio. It's a no-code platform that lets you build custom AI-powered communication workflows for your customers: intelligent virtual agents, automated call flows, AI-assisted interactions. You don't need a development team to build it. And once you build one, you can replicate it across your customer base. That's a services revenue stream that doesn't exist in anyone else's partner program at this level."

**Proof Point:**
8x8 customers in regulated industries have simplified their vendor environment and improved customer satisfaction scores within 18 months of deployment. We have customer stories ready to share.

**Call to Action:**
"Let's identify 2–3 accounts in your pipeline where this fits — I'll get you the competitive positioning guide and arrange an 8x8 SE for the first conversation."
...
- Offer a proof point — reference customer, customer story, or a proof of concept with 8x8 SE support
- Propose a small, low-risk next step to reduce perceived implementation concern

<!-- MANAGER_NOTES -->

### Suggested Call to Action
1. Register the target accounts in the 8x8 Partner Portal for deal protection
2. Request an 8x8 SE for the first customer conversation
...
|---|--------|-------|---------|
| 1 | Decide: ${decisions} | 8x8 Leadership | Immediate |
...
| 4 | Confirm next update cadence | All | This meeting |`;
  ...
  return `## 8x8 Executive Update — ${topic}
...
4. Share this update with relevant 8x8 stakeholders as needed`;
}
...
**Win theme:** Lead with the platform advantage — one vendor for employee communications and customer engagement, one SLA, no integration risk.`;
  ...
  if (modifier === "executive") {
    return `## Competitive Summary — 8x8 vs. ${competitor} at ${account}

**Deal:** ${deal}

**Situation:** ${competitor} has an established presence. Our champion is strong but we lack exec-level access.

**Win Strategy:** Reframe the conversation from vendor credibility to platform risk. ${competitor}'s architecture requires separate products for employee communications and the contact center. 8x8 brings those experiences together on a single platform with a single SLA — for a risk-averse customer in a regulated industry, that's a material operational and compliance advantage.
...
| # | Action | Owner | Urgency |
|---|--------|-------|---------|
| 1 | Get exec-level introduction to economic buyer | 8x8 Channel + Partner | Immediate |
| 2 | Request competitive deal desk support and pricing flexibility | 8x8 Partner Manager | This week |
| 3 | Deliver updated competitive positioning guide to partner | 8x8 Channel Marketing | 48 hours |
| 4 | Schedule customer reference call with comparable 8x8 win | 8x8 SE / PM | This week |
| 5 | Reframe evaluation criteria to emphasize platform integration and customer experience outcomes | Partner SE + 8x8 SE | Before next customer call |`;
  }
  ...
  return `## Competitive Intelligence — 8x8 vs. ${competitor}
**Account:** ${account}
...
### 8x8 Differentiators vs. ${competitor}

${isGenesys ? `**The strategic reframe — one platform vs. best-in-class contact center:**
Genesys built its reputation in the contact center. 8x8 is built to connect the entire business — employees, agents, and customer-facing teams — on a single communications and customer engagement platform. This is not a feature comparison. It's a different question: does the customer want the most sophisticated contact center on the planet, or do they want to modernize communications across the business and improve customer experience without creating another stack to manage?

For mid-market and enterprise customers who are managing separate UC and CC platforms today, 8x8's answer changes the economics. One admin model. One user identity. One reporting layer. One vendor relationship. One SLA.

**"Why run two clouds?"**
Most organizations evaluating Genesys are still buying UC from one vendor and contact center from another — with separate analytics, separate administration, and separate AI on top. Genesys solves the contact center half. 8x8 solves the whole equation. That becomes especially compelling when contact center agents need to collaborate with back-office employees, when sales and service teams need shared customer context, and when IT leadership has been asked to reduce vendor complexity.

**The Microsoft Teams angle:**
For customers already invested in Microsoft 365 and Teams, 8x8 has a strong native integration story — keeping Teams for collaboration while adding enterprise voice, contact center, and customer engagement through 8x8. This is core 8x8 differentiation. Genesys supports Teams but it is not central to their strategy.

**Where to concede — and why it actually helps:**
If the customer wants the most complex enterprise contact center with advanced routing orchestration and large-scale digital journey design, Genesys is a legitimate choice. Saying that is credible and disarming. Then immediately ask: how many of those advanced features will agents actually use on day one? How many admins will manage the environment? Is reducing vendor count a priority for the CIO? If the answers point toward simplicity, adoption, and speed — that's the 8x8 conversation.` :
isRingCentral ? `**Integrated platform — one vendor for employee communications and customer engagement:**
8x8 brings together 8x8 Work for employee communications, 8x8 Contact Center for agent and customer experiences, and 8x8 Engage for AI-powered customer interactions — all on a single platform with one admin console, one SLA, and one support relationship.
...
${competitor} is a contact center platform. It does not provide employee communications. A customer who buys ${competitor} still needs a separate UC solution — which means two vendors, two contracts, two support relationships, and data that never connects. 8x8 brings employee communications and customer engagement together on one platform, eliminating that complexity entirely.` :
`**Integrated platform — employee communications and customer engagement in one place:**
8x8 brings together 8x8 Work for employee communications, 8x8 Contact Center for agent and customer experiences, and 8x8 Engage for AI-powered customer interactions — all on a single platform with one admin console, one SLA, and one support relationship. ${competitor} requires separate products stitched together, creating integration complexity, split accountability, and two points of failure.`}

**Reliability & compliance:**
8x8 offers a 99.999% uptime SLA covering both employee communications and the contact center on the same platform. For regulated industries, a single platform with a single compliance posture is a material advantage.

**Total cost of ownership:**
Customers who consolidate on 8x8 typically reduce their communications vendor count significantly and lower total cost of ownership by 25–40% over 3 years. ${competitor}'s pricing often masks the long-term costs of integration, administration, and managing multiple vendor relationships.

**8x8 AI Studio — a differentiated capability:**
8x8 AI Studio is a no-code platform that enables partners and customers to build custom AI-powered communication workflows — intelligent virtual agents, automated call flows, and AI-assisted interactions — natively on the 8x8 platform. For technically capable partners, AI Studio creates a recurring services revenue stream that goes well beyond the initial platform sale.
...
4. Offer proof: Request a reference call with an 8x8 customer who chose 8x8 over Genesys — specifically one who had the same simplification mandate.` :
`1. Validate the concern — don't be defensive
2. Reframe the risk: "The real risk isn't vendor size — it's platform fragmentation. If employee communications and the contact center are on different platforms, who's accountable when the customer experience breaks? With 8x8, there's one answer to that question."
3. Offer proof: Request a customer reference call with an 8x8 win in the same industry
...
- Get 8x8 executive engagement with the economic buyer — ${isGenesys ? "the CIO or CFO, not just the contact center lead" : "before the RFP closes"}
...
- Use the 8x8 reference customer program — a peer conversation in the same industry is more powerful than any positioning document`;
}
...
${prospect}'s practice is exactly the profile we look for in a new 8x8 partner — ${whyFit}

I'd love 20 minutes to show you how partners like yours are building a profitable customer communications practice with 8x8. Would next week work?

[Your Name] | 8x8 Elevate Partner Program`;
  }
...
**Why now:** ${whyFit}
**Recommended approach:** Lead with the revenue opportunity and the customer engagement gap in their current portfolio. Position 8x8 as the natural complement to what they already sell — not a replacement.
**Proposed first step:** 30-minute intro call with 8x8 channel leadership + demo of the partner portal and margin structure.`;
  }
...
| # | Action | Owner | Due |
|---|--------|-------|-----|
| 1 | Send initial outreach email to ${contact} | 8x8 Partner Manager | Today |
| 2 | Research ${prospect}'s current customer base and key verticals | 8x8 PM | Before first call |
| 3 | Prepare customized margin and SPIFF overview | 8x8 Channel Ops | Before first call |
| 4 | Schedule 30-min intro call and 8x8 platform demo | Both | Within 2 weeks |
| 5 | Follow up with 8x8 Elevate Partner Program agreement and onboarding plan | 8x8 PM | Post-call |`;
  }
...
**Subject:** A revenue opportunity worth 30 minutes — ${prospect} + 8x8

Hi ${contact.split(",")[0].split(" ")[0] || "[Name]"},

I'm a partner account manager at 8x8 and wanted to reach out directly. I've been looking at ${prospect}'s practice and I think there's a meaningful revenue opportunity here that's worth a conversation.
...
3. 8x8 Engage as a differentiated add-on — AI-powered customer engagement for financial services, healthcare, and retail accounts
4. Elevate Partner Program margin comparison — show Apex/Summit economics with Contact Center and Engage attach highlighted
5. Elevate Partner Program tier benefits — Peak tier within 90 days is achievable

---

### Suggested First Steps After Initial Outreach
- 30-minute intro call + 8x8 platform demo
- Review current pipeline for immediate co-sell opportunities
- Complete 8x8 Elevate Partner Program agreement and kick off enablement
